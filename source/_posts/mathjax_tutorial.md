...
本文为 MathJax 在 [Cmd Markdown](https://www.zybuluo.com/mdeditor?url=https://www.zybuluo.com/static/editor/md-help.markdown) 环境下的语法指引。Cmd Markdown 编辑阅读器支持 $\LaTeX$ 编辑显示支持，例如：$\sum_{i=1}^n a_i=0$，访问 [MathJax](http://meta.math.stackexchange.com/questions/5020/mathjax-basic-tutorial-and-quick-reference) 以参考更多使用方法。
...
- 显示：$$\sum_{i=1}^n \frac{1}{i^2} \quad and \quad \prod_{i=1}^n \frac{1}{i^2} \quad and \quad \bigcup_{i=1}^{2} \boldsymbol{R}$$
...
> **@wjllz** 补充两个东西... 我不知道应不应该放在这里... 但是感觉放其它地方也不大合适... `\top` `\bot` 表示 Complete Lattice 当中的概念.
...
> **@Flashiam** <span style="text-align: center; display: block">y杠 `\bar{y}` → $\bar{y}$ </span>
...
|\mathcal|数学字符|$\mathcal{O}$|\boldsymbol|加粗符号|$\boldsymbol{\alpha}$|

> **@woria** \mathcal{O} 数学字符
...
对于较新的浏览器（HTML5 & CSS3），额外的 124 种颜色将被支持：
...
个人补充：在表示面积分和体积分时下标建议使用 `\boldsymbol{S}` 和 `\boldsymbol{V}` 符号；对于多维函数的超体积，可使用 `\int \cdots \int`，如下面的例子所示。
...
\int\int\int_V f(x)\,dz\,dy\,dx & \iiint_{\boldsymbol{V}} f(x)\,{\rm d}z\,{\rm d}y\,{\rm d}x \\
\text{多重积分示例} & \int \cdots \int_{\boldsymbol{D}} f(x_1,x_2,\,\cdots\, ,x_n)\,{\rm d}x_1\cdots{\rm d}x_n
\end{array}
```
...
\mathrm{Bad} & \mathrm{Better} \\
\hline \\
\iiint_{\boldsymbol{V}} f(x){\rm d}z {\rm d}y {\rm d}x & \iiint_{\boldsymbol{V}} f(x)\,{\rm d}z\,{\rm d}y\,{\rm d}x
\end{array}
```
...
> 参考博客：https://www.zybuluo.com/codeep/note/163962#9%E5%A6%82%E4%BD%95%E8%BE%93%E5%85%A5%E6%9E%81%E9%99%90%E8%BF%90%E7%AE%97